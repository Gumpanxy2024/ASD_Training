document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI(result) {
        ...
        // 处理分心提醒
        if (result.show_reminder) {
            showDistractionReminder();
        }
        
        // 显示当前分心时间
        if (result.current_distraction_time && result.current_distraction_time > 5) {
            const timerEl = document.getElementById('distraction-timer');
            if (!timerEl) {
                const timer = document.createElement('div');
                timer.id = 'distraction-timer';
                timer.className = result.current_distraction_time >= 10 ? 'critical' : 'warning';
                document.querySelector('.video-container').appendChild(timer);
            }
            
            const timerElement = document.getElementById('distraction-timer');
            timerElement.textContent = `分心时间: ${result.current_distraction_time}秒`;
            timerElement.className = result.current_distraction_time >= 10 ? 'critical' : 'warning';
        } else if (document.getElementById('distraction-timer')) {
            document.getElementById('distraction-timer').remove();
        }
    ...
    function showDistractionReminder() {
        ...
        reminder.className = 'distraction-reminder';
        reminder.innerHTML = `
            <div class="reminder-content">
                <h2>警告！</h2>
                <p>您已经分心超过10秒了</p>
                <p>请保持注意力集中</p>
                <button id="dismiss-reminder">知道了</button>
            </div>
        `;

...
def process_frame():
    ...
    elif result['is_distracted'] and detector.is_currently_distracted:
        # 检查是否需要显示提醒
        if recorder.check_reminder_needed():
            result['show_reminder'] = True
        # 添加当前分心时间信息
        result['current_distraction_time'] = round(recorder.get_current_distraction_time(), 1)
